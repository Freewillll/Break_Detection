...
bool combine_linker(vector<QList<NeuronSWC> > & linker, QList<NeuronSWC> & combined)
{
    ...
    if (neuronNum<=0)
    {
        std::cout<<"the linker file is empty, please check your data."<<std::endl;
        return false;
    }
    V3DLONG offset = 0;
    combined = linker[0];
...
//    }
    ...
    std::cout<<"Done 1st DFS"<<std::endl;
...
//    v3d_msg(QString::number(thres));
    if(thres>=0){  // If distance threshold > 0: make new connections
        ...
        while(cur_group>1)
        {
            ...
            // Find the closest std::pair of nodes between group 1 and the rest.
            V3DLONG m1,m2;
...
bool export_list2file(QList<NeuronSWC> & lN, QString fileSaveName, QString fileOpenName)
{
    ...
    file.close();
    std::cout<<"swc/eswc file "<<fileSaveName.toStdString()<<" has been generated, size: "<<lN.size()<<std::endl;
    return true;
}
...
void connect_swc(NeuronTree nt,QList<NeuronSWC>& newNeuron, double disThr, QStringList& swcinfo)
{
    ...
    QList<V3DLONG> cand;
    QList<XYZ> canddir;
    QVector<int> childNum(nt.listNeuron.size(), 0);     //    nums of child
    QVector<int> connNum(nt.listNeuron.size(), 0);      //  nums of neighbor -1
    QList<V3DLONG> components;
    QList<V3DLONG> pList;         // parent of cur node index
    V3DLONG curid=0;             // nums of components
    for(V3DLONG i=0; i<nt.listNeuron.size(); i++){
    ...
    qDebug()<<"------debug-----nums of components"<<curid;
    swcinfo<<QString("nums of components: %1").arg(curid);
    ...
    for(V3DLONG cid=0; cid<curid; cid++){
        ...
        pstack.push(components.indexOf(cid));     // component(root) index in QList
        while(!pstack.isEmpty()){
            V3DLONG pid=pstack.pop();
            chid = -1;
            chid = pList.indexOf(pid,chid+1);         // child index
            while(chid>=0){
                pstack.push(chid);
                components[chid]=cid;            // component id
    ...
    for(V3DLONG i=0; i<childNum.size(); i++){
        if(connNum.at(i)<1){               //  root -1
            cand.append(i);
            //get direction
            V3DLONG id=i;               // cur index
            V3DLONG sid;
            if(childNum[id]==1){          // single child root
                sid = pList.indexOf(id);   // son index
            }else{ //tips
                sid = pList[id];       //  par index
            }
            tip_pair.push_back(std::make_pair(id,sid));    //  ( tip index , neighbour of tip index )
        }
    }


    qDebug()<<"-------debug--------nums of tips and roots"<<tip_pair.size();
    swcinfo<<QString("nums of tips and roots %1").arg(tip_pair.size());
...
//    qDebug()<<connNum.size()<<":"<<childNum.size()<<":"<<cand.size();
    ...
    for(std::multimap<double, QVector<V3DLONG> >::iterator iter=connMap.begin(); iter!=connMap.end(); iter++){
        ...
        while(tmpid>=0){                      // change component
    ...
    while(1){
        ...
        while(!pqueue.isEmpty()){
            ...
            if(newid[oid]>0){
                NeuronSWC tmpNeuron;
                tmpNeuron.n = newid[oid];    //  new id to old index
            ...
            if(nt.listNeuron.at(oid).pn>=0){
                V3DLONG opid = nt.hashNeuron.value(nt.listNeuron.at(oid).pn);    // par index
                if(newid.at(opid)<0){
                    pqueue.enqueue(opid);
            ...
            //child
            V3DLONG tmpid=pList.indexOf(oid);    // chid index
            while(tmpid>=0){
                if(newid.at(tmpid)<0){
                    pqueue.enqueue(tmpid);
                    newpn[tmpid]=newid[oid];  // new par ns.id
                    newid[tmpid]=curid++;
                }
...
bool connect_swc(NeuronTree& nt, double disthre, QStringList& swcinfo){
    ...
    if(!nt.listNeuron.isEmpty()){
        ...
        connect_swc(nt, sortListNeuron, disthre, swcinfo);
        nt.listNeuron.clear();
